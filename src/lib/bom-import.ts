...
export async function importBomFromCsv(csvFilePath: string): Promise<{ success: boolean; message: string }> {
  try {
    ...
    const sanitizeText = (text: string | undefined): string => {
      ...
      return text
        .replace(/[•●■◆♪★♠♣♥♦]/g, '*') // Replace bullets and symbols with asterisks
        .replace(/[✓✔]/g, 'Y') // Replace checkmarks with Y
        .replace(/[✗✘]/g, 'N') // Replace crosses with N
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (line) {
        const [partCode, location, description] = line.split(',').map(field => 
          sanitizeText(field.trim())
        );
  ...
  try {
    ...
    for (const entry of bomData) {
      if (entry.part_code && entry.location) {
        const sanitizedPartCode = sanitizeText(entry.part_code);
        const sanitizedLocation = sanitizeText(entry.location);
        ...
        try {
          ...
          console.error(`Error inserting BOM entry (${sanitizedPartCode}@${sanitizedLocation}):`, insertError);
        }
      }
    }
...
export async function importBomFromExcel(excelFilePath: string): Promise<{ success: boolean; message: string }> {
  try {
    ...
    // Define sanitize function once at the beginning
    const sanitizeText = (text: string | undefined): string => {
      if (!text) return '';
      // Remove or replace problematic Unicode characters
      return text
    ...
    for (let rowNumber = 2; rowNumber <= worksheet.rowCount; rowNumber++) {
      ...
      const rawPartCode = row.getCell(2).text;
      const rawLocation = row.getCell(3).text;
      const rawDescription = row.getCell(4).text || '';
      
      // Use the single sanitizeText function defined at the beginning of the function
